...
router.post(
  ...
  body("email").isEmail(),
  body("profession").isAlpha(),
  body("password").isLength({ min: 6, max: 12 }),
  body("confirmpassword").isLength({ min: 6, max: 12 }),
  ...
  async (req, res) => {
    ...
    try {
      ...
      const { name, email, password, phone, profession, confirmpassword } =
        req.body;
      console.log(req.body);
      const user = await User.findOne({ email });
    //   password.isLength({ min: 6, max: 12 })
      if (user) {
